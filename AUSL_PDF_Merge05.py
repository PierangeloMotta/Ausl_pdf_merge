...
walk_dir = ["D:\\_AUSL\\StampeCollegio201911\\ZimbraItems\\Inbox\GIORNALI DI CASSA AUSL ROMAGNA 3 TRIM 2019\\","D:\\Temp\\"]
...
for cartella in walk_dir:
	if not os.path.isdir(cartella):
		print "Attenzione la directory " + str(cartella) + " non esiste!!!"
		print('cartella (absolute) = ' + os.path.abspath(cartella))
		#sys.exit()
	contadir = contadir + 1
	...
	for root, subdirs, files in os.walk(cartella):
		...
		for filename in files:
			...
			if filename.endswith(".pdf") or filename.endswith(".PDF"):
				...
				logfilehandlde = open(logfile,"a+")
				logfilehandlde.write("Processo il file\t" + str(file_path) + "\t")
				...
				logfilehandlde.writelines("Pagine:\t" + str(numerodipagine) + "\n")  
				...
				c.drawString(30, 766, unicode(str(os.path.basename(os.path.normpath(root))),'1252'))
				...
				logfilehandlde.close()
	
	# Write to an output PDF document
	output = open(str(contadir).zfill(3) + "_document-output.pdf", "wb")
